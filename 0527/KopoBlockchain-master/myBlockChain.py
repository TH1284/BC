...
def insertTxdata(tx):
    pd.read_sql_query(
        "insert into txdata values ({}, '{}', {}, '{}', '{}', {}, '{}', '{}')".format(tx.commitYN, tx.sender, tx.amount,
                                                                                      tx.receiver, tx.uuid, tx.fee,
                                                                                      tx.message, tx.txTime), engine)
        ...
        with open(g_bcFileName, 'r', newline='') as file:
            ...
            for line in blockReader:
                if blockReader.line_num == last_line_number:
                    lastBlock = Block(line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[7])
...
def readBlockchain(mode='internal'):
        ...
        with open(txFilePath, 'r', newline='') as file:
            ...
            for row in txReader:
                if row[0] == '0':  # find unmined txData
                    line = txData(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7])
